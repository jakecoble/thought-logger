...
export async function checkAndGetApiKey(): Promise<{hasKey: boolean, message: string}> {
  const apiKey = await getApiKey();
  if (!apiKey) {
    return {
      hasKey: false,
      message: "No OpenRouter API key found. Please set your API key to enable text extraction."
    };
  }
  return {
    hasKey: true,
...
export async function saveOpenRouterApiKey(apiKey: string): Promise<{success: boolean, message: string}> {
  try {
