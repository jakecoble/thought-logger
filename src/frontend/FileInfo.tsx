...
export function FileInfo() {
  ...
  useEffect(() => {
    ...
    window.userData.listRecentFiles().then((files) => {
      ...
      setFilesByDate(groupBy(filteredFiles, file => file.split("/").slice(-1)[0].split(".")[0]));
    });
  }, []);
  ...
  return (
    <div>
      ...
      <div className="flex gap-5 mt-5">
        <div className="min-w-[220px] max-w-[250px] pr-2 border-r border-gray-200 text-sm" style={{maxHeight: 600, overflowY: 'auto'}}>
        ...
        <div className="min-w-[300px] flex-1">
          {(() => {
            ...
            const allDates = Object.keys(filesByDate).sort((a, b) => b.localeCompare(a));
              ...
              result.push(
                <div key={date} className="mb-2.5" id={`day-${date}`}>
                  <div className="mb-1 font-bold flex items-center">
                    ...
                    {files.find(f => !isAiSummary(f) && !f.includes('.processed.')) &&
                      <button
                        className="ml-2 px-2 py-0.5 text-xs text-blue-800 font-normal"
                        onClick={() => window.userData.openExternalUrl(`file://${files.find(f => !isAiSummary(f) && !f.includes('.processed.'))}`)}
                      >raw</button>
                    }
                    {files.filter(f => !isAiSummary(f) && f.includes('.processed.')).map(f => (
                      <button
                        key={f}
